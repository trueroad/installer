...
sub check_for_old_updmap_cfg {
  chomp( my $tmfsysconf = _decode_console_out(`kpsewhich -var-value=TEXMFSYSCONFIG`) ) ;
  my $oldupd = "$tmfsysconf/web2c/updmap.cfg";
  return unless -r _encode_locale_fs($oldupd);  # if no such file, good.

  open (OLDUPD, _encode_locale_fs("<$oldupd")) || die "open($oldupd) failed: $!";
  my $firstline = <OLDUPD>;
  close(OLDUPD);
  ...
  if ($firstline =~ m/^# Generated by (install-tl|.*\/tlmgr) on/) {
    ...
    if (-r _encode_locale_fs($nn)) {
      my $fh;
      my $nn_raw;
      ($fh, $nn_raw) = tl_tmpfile(
        "updmap.cfg.DISABLED.XXXXXX", DIR => _encode_locale_fs("$tmfsysconf/web2c"));
      $nn = _decode_locale_fs($nn_raw);
    }
...
to
  $nn
";
    if (rename(_encode_locale_fs($oldupd), _encode_locale_fs($nn))) {
      if (system("mktexlsr", _encode_locale_fs($tmfsysconf)) != 0) {
